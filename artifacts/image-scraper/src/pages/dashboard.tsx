              ...
              {visibleCount < filteredImages.length ? (
                <>
                  <p className="text-xs text-muted-foreground">
                    Showing {visibleCount} of {filteredImages.length}
                  </p>
                  <Button
                    variant="outline"
                    className="gap-2"
                    onClick={() => setVisibleCount((n) => Math.min(n + PAGE_SIZE, filteredImages.length))}
                  >
                    <DownloadCloud size={15} />
                    Load more ({Math.min(PAGE_SIZE, filteredImages.length - visibleCount)} more)
                  </Button>
                </>
              ) : (
